...
PII detectors for busyclaw's redaction port (`Detector` from `@busyclaw/contracts`).
Two implementations of the one port, kept together so it proves itself without a stub:

- **`@busyclaw/detectors/regex`** — a deterministic, synchronous detector for the
...
- **`@busyclaw/detectors/presidio`** — the Microsoft Presidio analyzer behind the same
  port, for the categories regex cannot reach (names, locations). Async (HTTP `POST
  /analyze`), analyzer-only: the busyclaw redactor already owns the pseudonymization
  map, overlap resolution, and dedup, so this detector only *finds* spans.

Detection is **policy**, never mechanism — `@busyclaw/core` ships the redaction engine
