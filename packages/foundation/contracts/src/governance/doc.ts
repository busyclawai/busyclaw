// The busyclaw doc META CHANNEL (docs/plans/claw-client-plan.md, final doc-channel amendment).
...
// `{ busyclaw: { doc } }`, authored as plain typed `.configure({ busyclaw: { doc } })` through the
// global ArkEnv augmentation below (`busyclaw` as the key name is what makes global merging safe:
...
declare global {
	interface ArkEnv {
		meta(): {
			/** busyclaw's documentation channel — read by doc consumers (OpenAPI, CLI help,
			 *  tool catalog) via `docOf`; never part of validation error messages.
			 *  ADDITIVE-ONLY across busyclaw versions: mixed-version node_modules trees merge
...
export type DocSource = {
	readonly meta?: {
		...
		readonly busyclaw?: { readonly doc?: string };
	};
};

/**
 * The ONE doc reader, precedence built in: the rich `busyclaw.doc` channel wins, the `.describe()`
