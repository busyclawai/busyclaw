...
// every column helper and import path in it is Drizzle's; @busyclaw/cli only dispatches to it.
//
...
> = {
	pg: {
		...
		// TEXT, and WITHOUT `mode: "json"`: busyclaw's storage layer serializes a `json` field
...
function assertEmittable(provider: DrizzleProvider): EmittableProvider {
	...
	throw configurationError(
		`@busyclaw/storage-drizzle: no schema generator for the "${provider}" provider yet — pg and sqlite are mapped.`,
		{ provider },
	);
}
...
export function generateDrizzleSchema(options: DrizzleGenerateOptions): string {
	...
	const lines: string[] = [
		"// Generated by `busyclaw db generate` — do not edit by hand.",
