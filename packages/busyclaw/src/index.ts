...
> & {
	 ...
	 * this. Reserved `busyclaw:` scopes are dropped from whatever it returns.
	 *
...
type PluginCronFlag<Config> = Config extends {
	...
	? Plugin extends { $HasCron?: infer HasCron }
		? Extract<HasCron, BusyclawCronFlag>
		: "unknown-cron"
	: "no-cron";
...
export type Claw<Config extends RuntimeConfig = RuntimeConfig> = {
	 ...
	 *
	 * Here so `busyclaw db generate|migrate` can point at the module a host ALREADY has (the one
...
function hasCronTasks(plugins: readonly BusyclawPlugin[]): boolean {
...
function assertUniquePluginRoutes(plugins: readonly BusyclawPlugin[]): void {
	...
	for (const plugin of plugins) {
		for (const route of plugin.routes ?? []) {
			...
			if (previous) {
				throw configurationError("duplicate busyclaw plugin route", {
					key,
					previous,
					route: routeId,
				});
...
function collectPluginTools(input: {
	...
	for (const tool of contributed) {
		...
		if (pathOwner !== undefined) {
			throw configurationError("duplicate busyclaw plugin tool path", {
				path: tool.path,
		...
		if (nameOwner !== undefined) {
			throw configurationError("duplicate busyclaw plugin tool name", {
				name: tool.name,
				path: tool.path,
				pluginId: tool.pluginId,
				previous: nameOwner,
...
function configurePlugins(input: {
	...
	bind: (plugin: BusyclawPlugin) => PluginBoundContext;
	plugins: readonly BusyclawPlugin[];
}): BusyclawPlugin[] {
...
function assertApiContribution(input: {
	...
	if (
		...
		throw configurationError("busyclaw plugin api must be an object", {
...
function createPluginApi(input: {
	...
	for (const plugin of input.plugins) {
		...
		for (const [key, value] of Object.entries(api)) {
			...
			if (previous) {
				throw configurationError("duplicate busyclaw plugin api namespace", {
					namespace: key,
					pluginId: plugin.id,
					previous,
				});
			...
			if (key in input.baseApi) {
				throw configurationError(
					"busyclaw plugin api conflicts with base api",
					{
						namespace: key,
						pluginId: plugin.id,
					},
...
export function createClaw<const Config extends ClawConfig<RuntimeConfig>>(
	...
	// every boot/assembly warning through it — redaction (prefix "busyclaw redaction:"), secrets boot
	// (prefix "busyclaw secrets:"), and the event fan-out's observer-failure reports. `config.warn`
	...
	const models = getBusyclawModels({
	...
	const configuredPlugins = configurePlugins({
		...
		plugins: (config.plugins ?? []) as readonly BusyclawPlugin[],
	});
	...
	if (secretDeclarations.length > 0) {
		void validateSecretsAtBoot({
			...
			secrets,
			warn: (warning) => warn(`busyclaw secrets: ${warning.message}`),
		}).catch((err) => {
			warn(`busyclaw secrets: boot validation failed — ${errorMessage(err)}`);
		});
	}

	// Derived from the SAME merged field maps as `models` above (getBusyclawTables projects what
	// getBusyclawModels merges), so what the CLI migrates and what the entity adapter validates
...
export { govern } from "@busyclaw/runtime";
