...
import { endpoints, field, route } from "@busyclaw/contracts";
...
describe("createClaw plugin assembly", () => {
	...
	it("rejects duplicate plugin API namespaces", () => {
		...
		).toThrow(/duplicate busyclaw plugin api namespace/);
	});

	it("collects a plugin's owned tables through getBusyclawTables, beside core's", () => {
		const tables = getBusyclawTables({ plugins: [notesPlugin()] });
