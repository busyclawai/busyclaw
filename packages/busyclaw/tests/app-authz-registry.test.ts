...
import type { AccessGrant, PrincipalScope } from "@busyclaw/authz";
import { API_ACCESS_BASELINE, loadPolicyBundle } from "@busyclaw/authz";
import type {
	AccessGrantStore,
	ClawRunReadModel,
	ClawsStore,
...
describe("app-authz slice 5 — a team grant is dormant without scopes", () => {
	...
	it("a reserved busyclaw: scope from the host resolver is ignored, not honoured", async () => {
		...
		const rows = new Map<string, AccessGrant[]>([
			[
				"claw:claw-1",
				[{ principalRef: "busyclaw:uncontained:-", level: "manage" }],
			],
		]);
		...
		// learn about it.
		expect(warnings.join(" ")).toMatch(/reserved "busyclaw:" scope/);
	});
