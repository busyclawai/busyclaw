...
// (@busyclaw/authz) → rendered Cedar schema → live decisions through the governance chokepoint
// (@busyclaw/policy-cedar), including arg-conditioned permits and the needs-approval probe.
...
import type { JsonObject, ToolCall } from "@busyclaw/contracts";
import { createGovernance } from "@busyclaw/core";
import { toolsFromOpenApi } from "@busyclaw/runtime";
...
// test's unprefixed `principal` to the stamped `busyclaw__principal` the mapper now reads (audit #7:
