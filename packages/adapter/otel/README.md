...
Maps busyclaw's operational event stream onto OpenTelemetry spans. The bridge is just an
...
```ts
import { otelEvents } from "@busyclaw/otel"
import { trace } from "@opentelemetry/api"

const claw = createClaw({
  events: otelEvents({ tracer: trace.getTracer("busyclaw") }),
})
```
...
- one `invoke_agent <clawId>` root span per `runId`, closed by the terminal `run.*` event
  (a continuation gets a new root span; `busyclaw.run.id` is the cross-trace link)
...
- an `execute_tool <name>` child per tool call; denials close it with
  `busyclaw.reason_code`, approvals close it with `busyclaw.tool.outcome` (approvals can
