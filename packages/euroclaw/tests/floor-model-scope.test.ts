...
//
// `euroclaw.execute` is the exclusion, and it regressed once already. The floor builds its model in
...
import { EXECUTE_TOOL_PATH, SEARCH_TOOL_PATH } from "@euroclaw/runtime";
...
describe("what the floor models", () => {
	it("drops euroclaw.execute and keeps everything else", () => {
	...
	it("search IS modeled — the exclusion is one path, not the namespace", () => {
		// Excluding `euroclaw.*` wholesale would deny discovery: an action absent from the model is
